 ...
 * ---------------------------------------------------
 * 제품들이 올려져있거나 쌓여있는 창고의 선반은 각 창고마다 존재하기 때문에,
 * 구현은 멤버변수(실체변수)로 선언해야 한다.
 * ---------------------------------------------------
 * 메소드
 * 
 * 창고에 입고시키는 기능
 * 창고에서 출고시키는 기능
 * void set (Product product) : 창고에 있는 제품 1개의 정보를 수정하는 기능
 * Product get (Product product) : 창고에 있는 제품 1개의 정보를 출력하는 기능
 * void add (Product product) : 창고에 신제품을 새로 들여놓는 기능
 * void remove (Product product) : 더 이상 판매하지 않는 제품을 폐기하는 기능
 * Product[] getAllProducts () : 창고에 들어있는 전체 제품 목록을 가져오는 기능
 * 
 * @author PC38208
 *
 */
public class Warehouse {
	...
	/**
	 * 파라미터로 전달된 정보와 일치하는 제품이 있는지 검색하여
	 * (prodCode 필드가 같으면 같은 제품으로 처리)
	...
	private Product findProduct(Product product) {
		...
		for (Product prod: products) {
			// 배열(선반)에 존재하는 제품의 정보(prod)가 매개변수로 넘겨진 제품 정보(product)와 일치하는지 비교 : prodCode만 비교.
