...
public class ShoeTest {
	...
	public static void main(String[] args) {
		...
		System.out.println("Shoes List 방식 시작...");
		...
		// SH003에 덮어쓰기
		for (Shoe sh : shoes) {
			if (sh.equals(newShoe)) {
				shoes.set(findShoeIdx(shoes, "SH003"), newShoe);
				break;
			}
		}
		...
		// SH003에 덮어쓰기
		for (Shoe sh : shoes) {
			if (sh.equals(zeroQuantityShoe)) {
				shoes.set(findShoeIdx(shoes, "SH003"), zeroQuantityShoe);
				break;
			}
		}
		...
		// 7. SH003번 코드로 등록된 신발 정보를 삭제
		shoes.remove(findShoeIdx(shoes, "SH003"));
	...
	static int findShoeIdx (List<Shoe> shoes, String shoeCode) {
		int index = -1;

		for (int idx = 0; idx < shoes.size(); idx++) {
			if (shoes.get(idx).equals(shoeCode)) {
				index = idx;
				break;
			}
		}
